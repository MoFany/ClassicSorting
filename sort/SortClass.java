 ...
 * @author MoFany-J
 * @date 2022/9/12
 * @description Sort 该类实现自Sort接口,类方法的作用都是实现降序排序
 */
class SortClass extends SortUtility implements Sort {
    /**
     * 1.选择排序：加入标志量flag优化排序
     * 原理：
    ...
    /**
     * 3.快速排序：由于是降序排序，利用递归排序，所参考的基准值要么是数组首元素要么是数组尾元素
     * 原理：
    ...
    /**
     * 4.插入排序：选取第一个数作为参考标准，左侧为有序序列，右侧为待排序序列；
    ...
    public void insertionSort(int[] array) {
        ...
        for (int unorderedFirstIndex = 1; unorderedFirstIndex < array.length; unorderedFirstIndex++) {
            totalDegree = unorderedFirstIndex;
            //暂存当前无序序列中第一个数
        ...
        System.out.println("已有序，共执行了" + totalDegree + "趟插入排序！");
    }
    ...
    public void shellSort(int[] array) {
        ...
        System.out.println("已有序，共执行了" + totalDegree + "趟希尔排序！");
    }
    ...
    public void radixSort(int[] array) {
        ...
        System.out.println("已有序，共执行了" + totalDegree + "趟基数排序！");
    }

    /**
     * 8.桶排序（一种伪排序，将待排序数分为多个指定范围，即多个桶，让其保持桶间有序桶内也有序）
     * 原理：桶排序每个桶存储一定范围的元素（动态的计算桶数这也很重要？），
     *     然后通过映射函数，将待排序数组中的相关元素映射到各个对应的桶中，
     *     对每个桶中的元素进行排序（桶内的元素又利用何种排序算法进行排序？这至关重要），
     *     最后将非空桶中的元素逐个放入原序列中。
     *     桶排序需要尽量保证元素分散均匀，否则当所有数据集中在同一个桶中时，桶排序失效。
     *
     * 关于每个桶的区间范围的计算公式（桶的数量假定为待排序序列的数量）：
     * 桶的数量=Array.length;
     * 区间跨度 = （最大值-最小值）/ （桶的数量 - 1）;
     *
     * 缺点：
